...
const Finduser=()=>
{
    ...
    const updateData=()=>
    {
         Axios.put(`http://localhost:5000/updateUser/`,{id:id,newName:newName,newPassword:newPassword});
    }
    return <div className='modify-section'>
             <h1>Modify User</h1>
            <form className='modify-user-form'  >  
            <MDBInput id='form1' type='text' className='w-25 m-3 text-center'  placeholder={userData.name} onChange={(event)=>
            {
                setNewName(event.target.value);
            }} />
            <MDBInput id='form1' type='text' className='w-25 m-3 text-center' value={userData.email} />
            <MDBInput id='form1' type='text' className='w-25 m-3 text-center' placeholder={userData.password}  onChange={(event)=>
            {
